...
accession_names <- vector(length=length(unique(TE_intact$new_name)), mode ="character")

setwd("~/katherine/009REPEAT/002Fasta/001ATHILA")

for (i in 1:66){
  accession_names[i] <- paste(unique(TE_intact$new_name)[i],".csv",sep="")
  TE_intact %>% filter(new_name == unique(new_name)[i])
    write.csv(TE_intact, accession_names[i], quote=FALSE, row.names=FALSE)
}
...
setwd("/home/irh25/katherine/009REPEAT/002Fasta/001ATHILA")
...
for (i in 1:66) {
  ...
  setwd("/home/irh25/katherine/66_genomes_datafiles")
  ...
  #sorting out names
  my.out <- gsub("\\..*","_Athila_Seq.fa",myfiles[i])
